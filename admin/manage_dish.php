	...
	if(mysqli_num_rows(mysqli_query($con,$sql))>0){
		...
		if($id==''){
			if($type!='image/jpeg' && $type!='image/png'){
				...
				$did=mysqli_insert_id($con);   //gives last record's id
			...
			if($_FILES['image']['name']!=''){
				if($type!='image/jpeg' && $type!='image/png'){
					...
					$image=rand(111111111,999999999).'_'.$_FILES['image']['name'];
					//FOR UPLOADING A FILE IN PHP   ---> updating image
					move_uploaded_file($_FILES['image']['tmp_name'],SERVER_DISH_IMAGE.$image);
					...
					unlink(SERVER_DISH_IMAGE.$oldImage); //in this function image's server path has been given to delete old image after image updating
				}
			}
			if($image_error==''){
...
<div class="row">
                ...
                <div class="card-body">
                  <form class="forms-sample" method="post" enctype="multipart/form-data">
					...
					?>
						<div class="row mt8">
								<div class="col-5">
									<input type="hidden" name="dish_details_id[]" required value="<?php echo $dish_details_row['id'] ?>">
					...
					<button type="button" class="btn badge-danger mr-2" onclick="add_more()">Add More</button>  <!-- add_more() = javascript function -->
                  </form>
                </div>
              </div>
            </div>

         <!-- two ways for adding Add More-->   
		 <!-- two ways for adding Add More &     (1ST WAY) 
		</div>
		 <script>
			function add_more(){
        ...
        <!-- two ways for adding Add More         (2ND WAY)-->
		</div>
		<input type="hidden" id="add_more" value="1"/>
		 <script>
			function add_more(){
				var add_more=jQuery('#add_more').val();    //#add_more provides id of add_more
				add_more++;
			...
			function remove_more_new(id){
				...
				if(result==true){
					var cur_path=window.location.href;    //window.location.href => gives the current URL
